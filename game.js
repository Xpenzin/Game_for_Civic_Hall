...
function renderResult(won) {
  ...
  const text = won
    ...
    : `${MAX_BREACHES} threats broke through on wave ${state.currentWave} of ${lvl.name}, and the network went down. Real security works the same way — one defense alone isn't enough, and some threats can knock a single layer offline. Mix defenses and upgrade your key chokepoints.`;
  overlayBox.innerHTML = `
    <h2>${title}</h2>
    <p>${text}</p>
