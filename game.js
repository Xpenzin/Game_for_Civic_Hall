...
const towerTypes = {
  ...
  antivirus: {
    ...
    desc: 'Scans for known malicious signatures and can quarantine an infected host in place. Strong vs Trojans. Upgrading raises damage and the chance to fully root (quarantine) a target.',
    levels: [
  ...
  encryption: {
    ...
    desc: 'Deep packet inspection that ignores evasion tactics and flags anomalies across nearby traffic (splash damage). Strong vs Ransomware and novel Zero-days. Expensive and slow, but hardest to take down.',
    levels: [
...
const enemyTypes = {
  phishing: {
    label: 'Phishing Email', icon: '🎣', hp: 40, speed: 95, bounty: 8, breachDamage: 1,
  ...
  trojan: {
    label: 'Trojan', icon: '🐴', hp: 70, speed: 62, bounty: 12, breachDamage: 1,
  ...
  worm: {
    label: 'Worm', icon: '🪱', hp: 55, speed: 78, bounty: 10, breachDamage: 1,
  ...
  ddos: {
    label: 'DDoS Packet', icon: '📶', hp: 26, speed: 120, bounty: 6, breachDamage: 1,
  ...
  ransomware: {
    label: 'Ransomware', icon: '🔒', hp: 170, speed: 42, bounty: 26, breachDamage: 2,
  ...
  zeroday: {
    label: 'Zero-day Exploit', icon: '💀', hp: 520, speed: 34, bounty: 150, breachDamage: 5,
    weakness: null, resistance: 'all',
    disables: 'any', disableRange: 100, disableChance: 0.7, disableInterval: 900, disableDuration: 3200,
    tip: 'Zero-days exploit unknown vulnerabilities with no existing patch, and can knock out any single defense. Real defense: layered, defense-in-depth security so no one failure means total compromise.'
  }
};
...
function renderResult(won) {
  ...
  const title = won ? 'Network Secured! 🎉' : 'Server Breached 💥';
  const text = won
    ? `You survived all ${lvl.waves.length} waves on ${lvl.name} using layered, defense-in-depth security. That's exactly how real security teams stop attackers: no single tool catches everything.`
    : `Your server went down on wave ${state.currentWave} of ${lvl.name}. Real security works the same way — one defense alone isn't enough, and some threats can knock a single layer offline. Mix defenses and upgrade your key chokepoints.`;
  overlayBox.innerHTML = `
    <h2>${title}</h2>
    <p>${text}</p>
    <p class="highscore-line">Best on ${lvl.name}: Wave ${best}/${lvl.waves.length}</p>
    <div style="display:flex; gap:10px; justify-content:center; margin-top:10px;">
      <button class="overlay-btn" id="btn-retry">Retry This Network</button>
...
function breachEnemy(enemy) {
  ...
  addIntel(`BREACH: ${enemy.def.label} reached the server!`, enemy.def.tip, 'breach');
  const idx = state.enemies.indexOf(enemy);
  if (idx !== -1) state.enemies.splice(idx, 1);
  if (state.lives <= 0) {
    state.lives = 0;
...
function applyDamage(tower, def, lvl, target) {
  ...
  if (tower.type === 'encryption') {
    mult = target.def.weakness === 'encryption' ? 1.8 : 1.0; // ignores resistance
  } else {
    mult = damageMultiplier(target.def, tower.type);
  }

  if (tower.type === 'spamfilter' && target.def.weakness === 'spamfilter' && Math.random() < lvl.instaKillChance) {
    target.hp = 0;
  } else {
